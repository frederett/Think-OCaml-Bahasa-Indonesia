...
\newcommand{\thetitle}{Think OCaml: Berpikir (Fungsional) Seperti Ilmuwan Komputer}
\newcommand{\theversion}{0.1.1-frederett}
...
\begin{spacing}{3}
{\huge Think OCaml}\\
{\Large Berpikir (Fungsional) Seperti Ilmuwan Komputer}
\end{spacing}
...
{\small
Hak Cipta \copyright~2011 Nicholas Monje.
...
\item[April 2002:] Edisi pertama dari {\em How to Think Like
a Computer Scientist}.

\item[Agustus 2007:] Revisi Major, perubahan judul menjadi
{\em How to Think Like a (Python) Programmer}.

\item[Juni 2008:] Revisi Major, perubahan judul menjadi
{\em Think Python: How to Think Like
a Computer Scientist}.

\item[Januari 2011:] Terjemahaan untuk OCaml, perubahan judul menjadi
{\em Think OCaml: How to Think Like a Computer Scientist}.

\item[Juni 2022:] Terjemahaan buku ke dalam Bahasa Indonesia secara kolektif di \url{github.com/frederett/Think-OCaml-Bahasa-Indonesia}.
...
\section*{Daftar Kontributor}

Terima kasih banyak untuk Anumpana Kinaga atas izin penggunaan gambar kover buku.
...
Tujuan pada buku ini untuk mengajari anda berpikir seperti ilmuwan komputer. 
Cara berpikir ini menggabungkan berbagai pendekatan dari 
matematika, ilmu rekayasa teknik, dan ilmu alam. Layaknya matematikawan, ilmuwan komputer 
menggunakan bahasa formal untuk menyatakan ide (terutama ide-ide komputasi). Seperti rekayasawan,
mereka mendesain barang, merakit komponen menjadi sistem dan memeriksa kelebihan kekurangan desain 
dari berbagai alternatif yang ada. Mirip dengan ilmuwan, mereka mengamati perilaku sistem
 yang kompleks, membangun hipotesa dan menguji prediksi dari hipotesa.
...
Kemampuan tunggal yang paling penting untuk dimiliki ilmuwan komputer adalah 
kemampuan {\it problem solving} (penyelesaian masalah).
{\it Problem solving} berarti kemampuan untuk merumuskan masalah, 
berpikir kreatif menemukan solusi, dan menyatakan solusi dengan jelas dan akurat.
Ternyata, proses mempelajari membuat program adalah kesempatan yang bagus sekali 
untuk mengasah kemampuan {\it problem solving}. Ini alasannya mengapa, bagian ini dinamai, \textbf{Caranya memrogram} (membuat program).
...
Pada tingkat selanjutnya, anda akan belajar cara untuk memprogram, kemampuan yang berguna dengan sendirinya.
Pada tingkatan lain, anda akan menggunakan pemrogramman sebagai alat mencapai tujuan akhir. 
Seiring kita menelusuri buku ini lebih dalam, tujuan itu akan menjadi lebih jelas.

\section{Bahasa Pemrograman OCaml}
...
Bahasa pemrograman yang anda akan pelajari yaitu OCaml. OCaml adalah contoh dari
 {\bf bahasa tingkat tinggi} ({\it high-level language}), bahasa tingkat tinggi lain 
 yang mungkin pernah anda dengar yaitu C, C++, Perl, Java, dan Python.
...
Selain itu juga ada {\bf bahasa tingkat rendah} ({\it low-level language}), terkadang 
merujuk sebagai ``bahasa mesin'' ({\it machine language}) atau ``bahasa rakitan'' 
({\it assembly language}). Secara umum, komputer hanya bisa menjalankan program 
yang ditulis dalam bahasa tingkat rendah. Jadi, program yang ditulis dengan bahasa tingkat 
tinggi harus diproses sebelum ia dijalankan. Proses tambahan ini sedikit memakan waktu, yang menjadi
kekurangan kecil dari bahasa tingkat tinggi.
...
Banyak keuntungan dari bahasa tingkat tinggi. Pertama, lebih mudah memrogram 
dalam bahasa tingkat tinggi. Program yang ditulis dengan bahasa tingkat tinggi 
 memerlukan sedikit waktu untuk menulisnya, mereka memiliki penulisan yang pendek 
 dan mudah untuk dibaca. Kedua, bahasa tingkat tinggi bersifat {\bf portable}, berarti 
 mereka bisa berjalan di beragam jenis komputer dengan sedikit atau tanpa modifikasi. 
 Bahasa tingkat rendah hanya bisa berjalan di satu jenis komputer dan harus ditulis 
 ulang untuk berjalan di jenis lain.

Karena kelebihan ini, hampir semua program ditulis dengan bahasa tingkat tinggi.
Bahasa tingkat rendah hanya digunakan untuk beberapa aplikasi khusus.
...
Ada dua jenis program untuk memproses bahasa tingkat tinggi kedalam 
bahasa tingkat rendah : {\bf interpreter} dan {\bf kompilator}.
Interpreter membaca program (bahasa) tingkat tinggi dan menjalankanya, artinya 
melakukan apa yang tertulis didalam program. Ia memproses isi program satu per satu, 
membaca dan melakukan komputasi setiap baris kode ({\it source code}), proses ini disebut 
dengan interpretasi.
...
Kompilator membaca program dan menerjemahkan isinya sebelum program mulai dijalankan, 
proses ini disebut kompilasi. Pada konteks ini, program tingkat tinggi disebut sebagai 
{\bf kode sumber}, dan program hasil terjemahan disebut {\bf kode objek} ({\it object code}) 
atau {\it executable}. Setelah program dikompilasi, anda bisa menjalankan program berulang tanpa perlu 
penerjemaahan lagi.
...
Salah satu fitur unik dari OCaml, OCaml adalah bahasa pemrograman dengan pendekatan kompilasi
sekaligus interpretasi. Di sisi lain, OCaml dapat dikompilasi menjadi berkas {\it executable}.
Cara ini memudahkan anda untuk memberikan program kepada bukan pengguna OCaml, terutama pengguna baru,
untuk bahasa yang sedang naik daun seperti OCaml, ini adalah fitur bagus. Selain itu, OCaml bisa 
berlaku seperti bahasa pemrograman berbasis interpretasi, artinya skrip kode OCaml atau perintah 
OCaml bisa dijalankan melalui {\it interpreter}. Pendekatan ini membantu dalam pencarian galat ({\it error}) dalam kode anda atau membantu anda untuk menguji perintah bahasa dan potongan baris kode. Terdapat dua cara untuk menggunakan {\it interpreter} : mode interaktif dan mode skrip. Dalam mode interaktif, anda menuliskan perintah bahasa dalam {\bf toplevel} dan {\it interpreter} akan memunculkan hasil seperti ini:
...
Tanda nomor ({\it octothorp}), {\tt \#}, adalah masukan perintah yang menandakan {\it interpreter} siap menerima kode. Dua titik koma menandakan akhir baris. Jika anda ketik {\tt 1 + 1;;} kemudian tekan tombol {\bf enter}, {\it interpreter} akan membalas {\tt - : int = 2}.
...
Cara lainya, anda bisa menyimpan kode ke dalam sebuah berkas dan menggunakan {\it interpreter} untuk menjalankan isi berkas, yang disebut sebagai {\bf skrip}. Berdasarkan aturan, skrip OCaml memiliki nama dengan ekstensi akhiran {\tt .ml}.
...
Berkerja dalam mode interaktif cukup nyaman untuk menguji beberapa baris kode karena anda bisa mengetik 
dan langsung mendapatkan hasilnya. Tetapi untuk kode dengan baris yang tidak sedikit, anda bisa menyimpan kode 
sebagai skrip agar anda bisa mengubah dan menjalankanya di lain waktu.

\section{Toplevel OCaml}
...
Mari luangkan waktu untuk mengerti cara mengunakan {\it toplevel}, 
mekanisme {\it Read-Eval-Print-Loop} \footnote{\textit{Read-Eval-Print-Loop} 
adalah istilah untuk menjalankan kode perintah sekaligus menampilkan hasilnya } di OCaml.
{\it Toplevel} memiliki perbedaan sedikit saat berkerja yang bergantung pada sistem 
operasi anda. Saya akan membahasnya mengunakan {\it toplevel} dalam Windows (7) 
dan Distribusi GNU/Linux  \footnote{Pengguna Mac mengikuti cara distribusi GNU/Linux.}
...
Satu hal perlu anda gunakan adalah perintah OCaml yang dikenal sebagai {\it directives}.
{\it Directives} memberitahu interpreter atau kompilator untuk melakukan tugas khusus, seperti 
memuat kode dari berkas atau keluar dari program. Daftar kumpulan {\it directives} bisa 
dilihat melalui \url{http://caml.inria.fr/pub/docs/manual-ocaml/manual023.html}. Satu contoh {\it directives}
 yang berguna yaitu {\tt \verb"#quit;;"}, yang tujuan sesuai dengan namanya (keluar dari program).

\index{Directive}
\index{Toplevel}

\subsection{Distribusi GNU/Linux}

% First off, you must install the ocaml package and its dependencies. Open the terminal and type:

Pertama, anda perlu memasang {\bf OPAM} ({\it OCaml Package Manager}). Buka terminal dan ketik:
...
Kedua, anda menyiapkan {\bf OPAM} dan memasang kompilator. Buka terminal dan ketik
(bila ada opsi {\tt opam to modify \~/.bashrc ?}, ketik y lalu tekan enter) :
...
Tanda dollar {\tt \$} menandakan barisan perintah, tidak perlu ditulis. 
Perintah {\tt opam init} untuk menyiapkan terminal pengguna untuk keperluan {\bt OPAM}. 
Perintah {\tt opam switch create 4.14.0} untuk memasang kompilator, versi kompilator pada contoh ini 4.14.0. 
Daftar versi kompilator yang tersedia bisa dilihat dengan perintah {\tt opam switch list-available}.
Setelah pemasangan selesai, anda bisa membuka {\it toplevel} dengan mengetik {\tt ocaml} pada terminal.
Ketika didalam {\it toplevel}, anda bisa membuka script menggunakan {\it directives} {\tt \#use "{\it namafile}";;} :
...
Perhatikan bahwa lokasi folder saat ini ({\it current directory} \footnote{{\bf current directory} adalah lokasi 
folder untuk OCaml mencari berkas yang akan dijalankan, diluar perfolderan standar OCaml}) ketika membuka {\it toplevel} 
akan berada di lokasi folder yang sama pada terimnal anda saat ini. Anda bisa mengubah lokasi folder dengan {\it directive} 
{\tt \#cd ``{\it namafolder}'';;}.
...
{\bf Program} adalah rangkaian instruksi yang menjelaskan cara melakukan sebuah komputasi. Komputasi 
merupakan hal matematis, seperti menyelesaikan sistem persamaan atau mencari akar polinomial, tetapi itu juga bisa berarti komputasi simbolik, seperti mencari dan mengganti teks dari dokumen atau mengompilasi sebuah program.
...
Detailnya terlihat berbeda dalam bahasa yang berbeda, tetapi beberapa instruksi dasar muncul di hampir setiap bahasa

\begin{description}

% \item[input:] Get data from the keyboard, a file, or some other device.

\item[input:] Mengambil data dari {\it keyboard}, berkas, atau perangkat lain.

% \item[output:] Display data on the screen or send data to a file or other device.

\item[output:] Menampilkan data ke layar atau mengirim data kedalam berkas atau perangkat lain.


% \item[math:] Perform basic mathematical operations like addition and multiplication.

\item[math:] Melakukan operasi matematika dasar seperti penjumlahan dan perkalian.

% \item[conditional execution:] Check for certain conditions and execute the appropriate sequence of statements.

\item[conditional execution:] Memeriksa kondisi tertentu dan menjalankan alur pernyataan sesuai kondisi.

% \item[repetition:] Perform some action repeatedly, usually with some variation.

\item[repetition:] Melakukan aksi pernyataan berulang, biasanya dengan beberapa variasi kondisi.
...
Percaya atau tidak, itu sudah semuanya. Setiap program yang anda pernah gunakan, tidak perduli betapa rumitnya, 
tersusun dari kumpulan instruksi seperti ini. Sehingga anda dapat melihat pemrograman sebagai proses memecah perkerjaan besar dan 
rumit menjadi kumpulan perkerjaan yang lebih kecil sampai cukup sederhana untuk dilakukan oleh salah satu instruksi dasar diatas.

\index{algorithm}

% That may be a little vague, but we will come back to this topic when we talk about {\bf algorithms}.

Mungkin kurang terbayang, tidak apa kita akan kembali ke topik ini ketika kita membahas tentang {\bf algoritme}.

\section{Apa itu Awakutu?}
\index{debugging}
\index{bug}
...
Pemrograman rawan dengan kesalahan. Kesalahan dalam permograman disebut dengan {\bf bugs} (kutu, dalam bahas indonesia) 
\footnote{Sebenarnya tidak benar bahwa itu dinamai ngengat yang ditemukan di relay komputer, meskipun itu membuat 
cerita yang agak lucu. Untuk informasi lebih lanjut tentang etimologi, lihat \url{http://en.wikipedia.org/wiki/Software_bug}.} 
dan cara untuk melacak {\bf bug} disebut dengan awakutu (meskipun lebih populer dengan nama {\it debugging}).
...
Ada tiga jenis kesalahan atau galat yang bisa terjadi pada program : kesalahan tata tulis ({\it syntax error}), 
galat \footnote{ kata galat adalah sinonim dari kesalahan} waktu proses ({\it runtime error}) dan kesalahan makna ({\it semantic error}). Cukup penting 
untuk membeda antara kesalahan program karena membantu kita untuk melacaknya sedini mungkin.

\subsection{Kesalahan Tata Tulis}
\index{syntax error}
\index{error!syntax}
\index{error message}
...
OCaml hanya akan menjalakan program bila tata tulisnya benar; sebaliknya, {\it interpreter} akan menampilkan pesan galat.
{\bf Tata tulis} ({\it Syntax}) merujuk ke struktur dari program dan aturan pada struktur program. Sebagai contoh, tanda kurung
harus berpasangan, jadi {\tt (1 + 2)} terbilang legal (sesuai peraturan), tetapi {\tt 8)} adalah kesalahan tata
 tulis (sebab tanda kurungnya tidak berpasangan).
...
Sebagai pembaca, kita bisa mentolerir kesalahan dalam tata tulis, seperti kenapa membaca tulisan dengan tipo tanpa 
memberikan pesan kesalahan. OCaml tidak begitu toleran. Jika ada satu kesalahan tata tulis didalam program anda, OCaml akan 
menampilkan pesan kesalahan dan program berhenti. Saat pekan pertama dalam karir pemrograman, anda akan banyak menghabiskan waktu 
untuk melacak kesalahan tata tuils. Seiring anda mendapatkan pengalaman, anda akan sedikit membuat kesalahan dan cepat dalam melacaknya.

\subsection{Galat Waktu Proses}
\label{runtime}
\index{runtime error}
\index{error!runtime}
\index{exception}
...
Jenis kesalahan program kedua yaitu galat waktu proses, sesuai namanya kesalahan ini hanya muncul setelah 
program dijalankan. Kesalahan ini juga disebut {\bf exceptions} (pengecualian) karena ia menunjukan sesuatu
yang luar biasa (dan buruk) telah terjadi.
...
Galat waktu proses sukar terjadi pada program sederhana yang akan anda temui di beberapa bab awal buku ini.

\subsection{Kesalahan Makna}
\index{semantics}
...
Jenis kesalahan ketiga yaitu Kesalahan Makna, sering juga disebut kesalahan logika. 
Jika program anda memiliki kesalahan makna, ia tetap berhasil berjalan seperti biasa dan tidak 
memunculkan pesan error, tetapi program tidak berkerja dengan benar. Ia melakukan hal lain.
Tentunya, ia harus berjalan sesuai dengan apa yang anda perintahkan.
...
Masalahnya adalah program yang anda tulis bukanlah program yang anda harapkan kerjanya. Ini berarti maksud 
yang tertulis di program salah. Mengenali kesalahan makna bisa terbilang cukup mengecoh karena kita perlu 
melacak mundur alur kerja program dengan melihat {\it output} dari program dan mencoba mencari tahu apa yang 
dilakukan program.

\subsection{Awakutu Experimental}
...
Salah satu kemampuan penting yang akan anda dapat yaitu awakutu. Memang itu bisa membuat frustasi, awaktu adalah 
bagian yang paling kaya secara intelektual, menantang, dan menarik dari programing.
...
Di satu sisi, awakutu seperti perkerjaan detektif. Anda berhadapan dengan petunjuk dan anda juga menyimpulan proses dan kejadian
yang membawa ke hasil yang anda lihat.
...
Awakutu juga nampak seperti ilmu eksperimen. Setiap anda memiliki ide tentang ada suatu yang salah, anda akan memodifikasi program 
dan mencobanya (mengawakutunya) lagi. Jika hipotesis\footnote{hipotesis adalah pendapat dari perkiraan bagaimana sesuatu berkerja}
anda benar, maka anda bisa memprediksi hasil dari modifikasi yang anda buat dan membimbing anda selangkah mendekat dengan program 
yang berkerja. Jika hipotesis anda salah, anda harus membuat hipotesis baru. Seperti yang Sherlock Holmes tunjukan, ``Ketika anda 
berhasil menghilangkan kemustahilan, apa yang tersisa, betapa tidak mungkinnya, pastilah kebenaran'' (A. Conan Doyle, {\em The Sign of Four})
...
Bagi sebagian orang, pemrograman dan awakutu dianggap sama. Oleh karenanya, pemrogramman adalah proses bertahap mengawakutu 
sebuah program sampai ia melakukan yang anda inginkan. Maksudnya, Anda patut memulai dengan program yang melakukan sesuatu 
dan membuat modifikasi kecil,sembari mengawakutunya, dengan begitu anda akan memiliki program yang berkerja sesuai harapan.
...
Sebagai contoh, Linux adalah inti sistem operasi yang berisi ribuan baris kode, tetapi hal tersebut dimulai dengan sesuatu 
yang sederhana yang digunakan Linus Torvalds \footnote{Linux Torvalds adalah penemu kernel Linux} untuk mengenal chip Intel 80386.
Dilansir oleh Larry Greenfield, ``Satu dari proyek awal Linus adalah program untuk yang akan beralih mencetak AAAA dan BBBB.
 Kemudian ia berevolusi menjadi Linux.'' ({\em The Linux Users' Guide} Beta Version 1)

\index{Linux}

% Later chapters will make more suggestions about debugging and other programming practices.

Bab selanjutnya akan memuat saran tentang awakutu dan praktik-praktik pemrograman lainnya.

\section{Bahasa Alami dan Formal}
...
{\bf Bahasa Alami} adalah bahasa yang digunakan manusia (bahasa yang kita pakai), seperti Bahasa Indonesia, Jawa, 
Lampung dan Inggris. Bahasa tersebut tidak dirancangan oleh orang (meskipun ada beberapa usaha untuk membakukanya); 
ia berevolusi secara alami.
...
{\bf Bahasa Formal } adalah bahasa yang dirancang oleh manusia untuk aplikasi khusus. Sebagai contoh, notasi 
yang dipakai dalam matematika adalah bahasa formal yang digunakan untuk mengambarkan hubungkan antara angka dan simbol.
Kimiawan menggunakan bahasa formal untuk mewakili struktur molekul kimia dan bagian terpenting:
...
\begin{quote}
	{\bf Bahasa pemrograman adalah bahasa formal yang didesain untuk menyatakan komputasi.}
\end{quote}
...
Bahasa formal cenderung memiliki aturan tata tulis yang ketat. Sebagai contoh, $3 + 3 = 6$ adalah pernyataan matematika
yang sah secara sintaksis\footnote{Sintaksis sinonim dari tata tulis}. $H_2O$ adalah formula kimia yang benar tetapi
$_2Zz$ tidak.
...
Aturan tata tulis memiliki dua pendekatan, berkaitan dengan token dan struktur. Token adalah elemen dasar dari sebuah bahasa,
seperti kata-kata, angka, dan unsur kimia. Salah satu contohnya yakni $3 + = 3 \mbox{\$} 6$, simbol $\$$ bukan token yang sah 
dalam matematika (yang saya tahu saat ini). Mirip dengan , $_2Zz$ yang bukan token sah dalam kimia karena tidak ada unsur 
dengan singkatan $Zz$.
...
Jenis kesalahan sintaksis kedua berkaitan dengan struktur pernyataan; cara penyusunan token. $3 + = 3 \mbox{\$} 6$ 
termasuk penyataan yang tidak sah, meskipun $+$ dan $=$ adalah token yang sah, masih terdapat token yang tidak sah 
setelahnya : \mbox{\$}. Serupa dengan formula kimia, subskrip nomor atom pada notasi unsur ditulis setelah nama 
unsur bukan sebelumnya.
...
\begin{ex}
Tulislah sebuah kalimat Bahasa Indonesia yang terstrukur dengan baik namun tanpa token yang sah. Kemudian tulis
pernyataan lain dengan token yang sah tetapi struktur yang tidak sah.
\end{ex}
...
Ketika anda membaca kalimat dalam Bahasa Indonesia atau sebuah pernyataan dalam bahasa formal, anda perlu mencari tahu
struktur dari pernyataan tersebut (meskipun dalam bahasa alami anda melakukanya dengan tidak sadar). Proses isi disebut
 dengan {\bf parsing} (penguraian).
...
Sebagai contoh, ketika anda mendengar kalimat, ``uang terjatuh'' anda mengerti bahwa ``uang'' adalah subjek
dan ``terjatuh'' adalah predikat. Setelah anda mengurai kalimat, anda bisa mengetahui maksud atau makna dari 
sebuah kalimat. Bayangkan anda tahu apa itu uang dan apa aritnya menjatuhkan, anda akan mengerti suratan 
umum dari kalimat ini.
...
Meskipun bahasa alami dan formal memiliki kesamaan fitur -- token, struktur, tata tulis, dan makna --
...
\item[Ambiguitas:] Bahasa alami penuh dengan ambiguitas, kita menangani ini dengan menggunakan petunjuk kontekstual 
\footnote{menghubungkan dengan situasi atau posisi} dan informasi lain. Bahasa formal di desain untuk sedikit atau 
tidak memiliki ambiguitas dalam berbahasa, yang berarti satu penyataan hanya memiliki satu makna, terlepas dari konteks apapun.
...
\item[Pengulangan:] Untuk mengatasi ambiguitas dan mengurangi kesalahpahaman, bahasa alami mengandalkan 
{\bf redundansi} (pengulangan). Hasilnya, ia sering bertele-tele. Bahasa formal lebih sedikit redundansi dan 
lebih ringkas.
...
\item[Makna literal:] Bahasa alami penuh dengan ungkapan dan metapora. Jika saya menyebut, ``uang terjatuh''
 ada kemungkinan tidak ada uang atau tidak ada sesuatu yang terjatuh \footnote{Ungkapan ini berarti bahwa 
 seseorang menyadari sesuatu setelah periode kebingungan}. Bahasa formal berarti persis dengan apa 
 yang dikatakan. 
...
Orang yang tumbuh dewasa dengan berbicara dalam bahasa alami -- semua orang yang saya kenal-- sering kesulitan 
menyesuaikan diri dengan bahasa formal. Di sisi lain, perbedaan bahasa alami dan formal seperti perbedaan 
antara pusisi dan prosa, tapi lebih dari itu:
...
\item[Puisi:] Kata-kata yang  bunyinya termasuk bagian dari makna, dan keseluruhan 
 rangkaian puisi menggugah rangsangan emosional. Keambiguitasaan banyak terjadi bahkan disengaja.
...
\item[Prosa:] Makna dari kata-kata lebih penting dan struktur memiliki penekanan lebih pada 
makna. Prosa lebih diterima untuk dianalisis \footnote{Analisis dalam konteks ini adalah memeriksa
 sebab-akibat dari suatu pemaknaan} ketimbang puisi meskipun masih sering ambigu.
...
\item[Program:] Makna dari program komputer bersifat tidak ambigu, tersurat, dan bisa dipahami 
seluruhnya dengan menganalisa token dan struktur program.
...
Berikut berapa saran saat membaca program (dan bahasa formal lainnya). Pertama, perlu diingat bahwa 
bahasa formal lebih padat dari bahasa alami, sehingga memakan waktu untuk membacanya. Kedua,
struktur bahasa sangatlah penting, bukan sebuah ide bagus untuk membacanya dari atas kebawah, kiri ke kanan.
Maka, anda perlu belajar mengurai isi program di luar kepala, seperti mengenali token-token dan mengenali 
struktur pada program. Terakhir, detail penting. Kesalahan kecil pada pembacaan dan tanda baca, yang bisa
dimaklumi dalam bahasa alami, bisa menyebabkan perbedaan besar dalam bahasa formal.

% \section{The first program}
\section{Program perdana}
\label{hello}
...
Umumnya, program yang anda tulis dalam bahasa baru dikenal ``Hello, World'' karena yang ia lakukan
hanya menampikan kata, ``Hello, World!'' di OCaml, salah satu contohnya terlihat seperti ini:
...
Ini contoh dari pernyataan {\bf print} (cetak), bukan berarti mencetak tulisan ke atas kertas.
Ia menampilkan nilai pada layar. Pada contoh ini, seperti ini hasilnya
...
Tanda petik dua pada program menandakan awal dan akhir dari teks yang akan ditampilkan; tanda ini tidak muncul 
saat ditampilkan. Karakter ``\verb"\n"'' disebut  {\bf escape character}  
\footnote{escape character adalah karater khusus didalam teks (\textit{string}) dilambangkan dengan garis miring terbalik 
(``\textbackslash'') diikuti dengan karakter atau rangkaian karakter yang menghasilkan karakter khusus, seperti baris baru.}, khusus
untuk karakter baris teks baru, yang menampilkan output untuk pindah ke baris bawah (baru).
...
Ada kata kunci yang lebih umum untuk mencetak kelayar disebut ``printf,'' yang membolehkan anda 
melakukan lebih dari  {\tt print\verb"_"string}. Printf berasal dari modulnya sendiri, yang disebut Printf, untuk 
memanggil \footnote{Memanggil dalam konteks ini maksudnya menggunakan sebuah fungsi yang ada dengan sebuah nilai argumen.} 
fungsi printf anda bisa mengetiknya dengan ``Printf.printf''\footnote{{\bf nama module}.{\it nama fungsi} seperti ini 
format mengguankan fungsi dari sebuah modul}. Modul adalah ekstensi pada OCaml yang banyak memiliki fungsi berguna.
Printf adalah modul standar ({\it standar library} pada bahasa lain), yang berarti modul tersebut sudah bawaan yang terinstall
 langsung dengan OCaml, jadi tidak perlu menginstallnya lagi. Anda dapat mencoba printf sendiri jika Anda tertarik.

% Alternatively, you can import the module by using the \verb"#use" directive:

Kalau tidak, anda bisa mengimpor modul menggunakan {\it directives} \verb"#use":
...
Kemudian panggil printf dengan mengetik {\tt printf}. Namun, mengimpor sebuah modul umumnya tidak disarankan kecuali 
anda akan menggunakan banyak fungsi yang ada pada modul tersebut.

\index{module}

% Really, we could simply just type ``Hello, World!'' and get the result ``{\tt - : string = ''Hello, World!''.}''

Sungguh, kita cukup mengetik ``Hello, World!'' dan mendapatkan hasilnya ``{\tt - : string = ''Hello, World!''.}''

% Some people judge the quality of a programming language by the simplicity of the ``Hello, World!'' program.  
% By this standard, OCaml does \footnote{Or can do, anyway. There are about as many different OCaml ``Hello, World!'' 
% programs as there are OCaml programmers, all arguably equally valid.} about as well as possible.

Sebagian orang menilai kualitas bahasa pemrograman dari kesederhanaan membuat program ``Hello, World!''.
Dengan standar ini, OCaml melakukan \footnote{Atau bisa juga, ada banyak variasi program ``Hello, World!'' 
dalam OCaml yang berbeda dengan programmer OCaml lainya, semuanya bisa dibilang sama-sama valid.} dengan sebaik mungkin.

\section{Fungsional, sebagai lawan dari ...?}
...
{\bf Pemrograman Fungsional} ({\it Functional Programming}) adalah paradigma pemrograman
yang dimana fungsi sebagai warga kelas utama. Sejauh ini, definisi ini terkesan aneh, tetapi itu yang bisa 
kita punya sekarang.
...
Paradigma bahasa pemrograman populer lainya adalah {\bf Orientasi-Objek} ({\it Object-Oriented}). 
Bahasa yang berorientasi objek, seperti C++ dan Python, cenderung berfokus memiliki ``objek'' 
dan mengubahnya. ``Objek'' adalah payung istilah untuk semua bentuk penyimpanan data yang berbeda. 

% OCaml is mostly a functional programming language. However, OCaml has support for some level of 
% object-oriented programming, unlike it's predecessor Caml. The ``O'' in OCaml stands for 
% ``Objective''.\footnote{``Caml'' once stood for ``Categorical Abstract Machine Language'' 
% but it doesn't any more. Now it just represents an even-toed ungulate with humps on its back.}

OCaml sebagian besar adalah bahasa pemrograman fungsional. Meskipun, OCaml memiliki dukungan untuk
 beberapa tingkat pemrograman berorientasi objek, tidak seperti pendahulunya Caml. O dalam OCaml memiliki 
 arti ``Objective''.\footnote{``Caml'' merupakan singkatan dari ``Categorical Abstract Machine Language''
 tetapi sekarang tidak lagi. Sekarang itu hanya mewakili ungulata berjari genap dengan punuk di punggungnya 
 alias Unta. Caml merupakan plesetan dari Camel yang berarti Unta dalam bahasa inggris}.
...
Ada baiknya bila anda membaca buku ini di depan komputer agar dapat mencoba contoh-contohnya sambil berjalan menyusuri isi buku ini. Anda bisa menjalankan 
keseluruhan contoh dalam mode interaktif interpreter, tetapi jika Anda menulis kode pada skrip, akan memudah anda untuk 
mencoba variasi gabungan dari contoh-contoh program dalam satu skrip.
...
Setiap kali Anda bereksperimen dengan fitur baru, Anda harus mencoba membuat kesalahan. Sebagai contoh, dalam program ``Hello, world!'', Apa yang terjadi bila anda melupakan satu tanda petik ? Apa yang terjadi tanpa tanda petik? Apa yang terjadi bila anda salah membaca {\tt print\verb"_"string} ?
...
Eksperimen seperti ini membantu anda untuk mengingat apa yang anda baca; dan juga membantu saat mengawakutu program, karena anda mengerti maksud dari pesan galat yang muncul. Bukan hal yang buruk membuat kesalahan saat ini dan disengaja daripada di kemudian waktu dan tidak terduga.

% Programming, and especially debugging, sometimes brings out strong emotions.  If you are struggling with a difficult bug, you might feel angry, despondent or embarrassed.

Saat memrograman dan khusunya mengawakutuan, terkadang proses ini memengaruhi emosi pada diri kita. Jika kita kesusahan dengan {\bf bug} yang rumit, memuncul merasa kesal, putus asa ataupun malu pada diri kita.
...
Ada sebuah bukti yang menyatakan kita secara alami menganggap interaksi komputer seperti interaksi dengan manusia \footnote{baca Reeves dan Nass, {\it The Media Equation: How People Treat Computers, Television, and New Media Like Real People and Places}.}. Ketika komputer berkerja dengan baik, kita anggap ia sebagai teman dan tetapi ketika mereka kasar, kita juga membalas dengan cara yang sama kasarnya.
...
Persiapan untuk menghadapi reaksi ini akan membantu anda untuk berdamai denganya. Satu pendekatan pemikiran bahwa komputer sebagai 
pekerja dengan kekuatan tertentu, seperti kecepatan dan akurasi, dan juga kelemahan, seperti kurangnya empati dan ketidak mampuan memahami gambaran besar suatu konteks.

% Your job is to be a good manager: find ways to take advantage of the strengths and mitigate the weaknesses.  And find ways to use your emotions to engage with the problem, without letting your reactions interfere with your ability to work effectively.

Tugas anda adalah menjadi manajer yang baik: mencari cara untuk mengunakan kelebihan dan memitigasi kekurangan. Selain itu, cari cara untuk mengunakan emosi anda untuk berhadapan dengan masalah, tanpa membiarkan reaksi anda menggangu kemampuan untuk berkerja efektif.
...
Belajar pengawakutuan bisa membuat kita frustasi, tetapi ia merupakan kemampuan yang penting untuk berbagai aktivitas dalam pemrograman. Pada akhir bab ada bagian terkait awaktu, seperti ini, dengan pandangan saya pribadi. Saya harap itu membantu!
...
\item[problem solving:]  Proses menyimpulkan masalah, mencari dan menyatakan solusi.
\index{problem solving}

% \item[high-level language:]  A programming language like OCaml that is designed to be easy for humans to read and write.
% \index{high-level language}

\item[high-level language:]  Bahasa pemrograman, seperti OCaml, yang didesain mudah dibaca dan ditulis oleh manusia.
\index{high-level language}
...
\item[low-level language:] Bahasa pemrograman yang didesain agar mudah dijalakan oleh komputer, disebuh juga sebagai ``bahasa mesin'' atau ``bahasa rakitan.''
\index{low-level language}

% \item[portability:]  A property of a program that can run on more than one kind of computer.
% \index{portability}

\item[portability:]  Karakter program yang bisa dijalankan lebih dari satu jenis komputer, CPU.
\index{portability}

% \item[interpret:]  To execute a program in a high-level language by translating it one line at a time.
% \index{interpret}

\item[interpret:]  Cara untuk menjalankan program bahasa tingkat tinggi dengan menerjemahkannya perbaris.
\index{interpret}
...
\item[compile:]  Menerjemahkan program yang ditulis dalam bahasa tingkat tinggi ke bahasa tingkat rendah, agar bisa dieksekusi lebih lanjut.
\index{compile}

% \item[source code:]  A program in a high-level language before being compiled.
% \index{source code}

\item[source code:] Kode program bahasa tingkat tinggi yang belum dikompilasi.
\index{source code}

% \item[object code:]  The output of the compiler after it translates the program.
% \index{object code}

\item[object code:]  Hasil keluaran kompilator setelah program diterjemahkan.
\index{object code}

% \item[executable:]  Another name for object code that is ready to be executed.
% \index{executable}

\item[executable:]  Nama lain untuk {\it object code} yang siap untuk dijalankan.
\index{executable}

% \item[prompt:] Characters displayed by the interpreter to indicate that it is ready to take input from the user.
% \index{prompt}

\item[prompt:] Karakter yang ditampilkan interpreter untuk menandakan, interpreter siap menerima ketikan/masukan pengguna.
\index{prompt}

% \item[script:] A program stored in a file (usually one that will be interpreted).
% \index{script}

\item[script:] Kode program yang disimpan dalam berkas (file).
\index{script}

% \item[interactive mode:] A way of using the OCaml interpreter by typing commands and expressions at the prompt.
% \index{interactive mode}

\item[interactive mode:] Cara menggunakan interpreter OCaml dengan mengetikan langsung perintah dan ekspresi pada prompt.
\index{interactive mode}

% \item[script mode:] A way of using the OCaml interpreter to read and execute statements in a script.
% \index{script mode}

\item[script mode:] Cara menggunakan interpreter OCaml dengan membaca dan menjalankan pernyataan dari skrip.
\index{script mode}

% \item[toplevel:] The interface you interact with when working with the interpreter.
% \index{toplevel}

\item[toplevel:] Antarmuka tempat anda menggunakan dengan interpreter.
\index{toplevel}

% \item[program:] A set of instructions that specifies a computation.
% \index{program}

\item[program:] Kumpulan instruksi yang menyatakan proses komputasi.
\index{program}

% \item[algorithm:]  A general process for solving a category of problems.
% \index{algorithm}

\item[algorithm:] Sebuah proses umum untuk memecahkan masalah.
\index{algorithm}

% \item[bug:]  An error in a program.
% \index{bug}

\item[bug:]  Kesalahan dalam program.
\index{bug}
...
\item[debugging / Awakutu:] Proses menemukan dan menghapus salah satu dari tiga jenis kesalahan pemrograman.
\index{debugging}

% \item[syntax:]  The structure of a program.
% \index{syntax}

\item[syntax:] Struktur dari Program.
\index{syntax}
...
\item[syntax error:]  Kesalahan dalam program yang membuatnya tidak bisa diterjemahkan ({\bf parse}) (dan juga tidak bisa dijalankan).
\index{syntax error}

% \item[exception:]  An error that is detected while the program is running.
% \index{exception}

\item[exception:]  Kesalahan yang muncul ketika program sedang berjalan.
\index{exception}

% \item[semantics:]  The meaning of a program.
% \index{semantics}

\item[semantics:]  Makna yang terkandung dalam program.
\index{semantics}

% \item[semantic error:]   An error in a program that makes it do something other than what the programmer intended.
% \index{semantic error}

\item[semantic error:]  Kesalahan makna dalam program yang membuat ia melakukan sesuatu yang lain dari yang dimaksudkan (diingikan) oleh programmer.
\index{semantic error}

% \item[natural language:]  Any one of the languages that people speak that evolved naturally.
% \index{natural language}

\item[natural language:] Salah satu bahasa yang digunakan orang yang berkembang secara alami.
\index{natural language}
...
\item[formal language:] Salah satu bahasa yang dirancang orang untuk tujuan tertentu, seperti mewakili ide matematika atau program komputer; semua bahasa pemrograman adalah bahasa formal.
\index{formal language}

% \item[token:]  One of the basic elements of the syntactic structure of a program, analogous to a word in a natural language.
% \index{token}

\item[token:]  Salah satu elemen dasar dari struktur sintaksis program, sama dengan sebuah kata dalam bahasa alami.
\index{token}

% \item[parse:]  To examine a program and analyze the syntactic structure.
% \index{parse}

\item[parse:]  Memeriksa isi program dan menganalisa stuktur sintaks didalamnya.
\index{parse}
...
\item[print statement:]  Instruksi untuk interpreter OCaml menampilkan nilai kelayar.
\index{print statement}
\index{statement!print}
...
\begin{ex}
Buka alamat ini di browser web anda : \url{ocaml.org}. Halaman web ini berisi informasi tentang OCaml dan halaman berguna terkait dengan OCaml, halaman ini memberi Anda petunjuk untuk mencari dokumentasi OCaml.
...
\begin{ex}
Buka {\bf toplevel} OCaml dan buat program ``Hello, World!'' dalam mode interaktif.

Sekarang buat skrip bernama ``hello.ml'' lalu tulis program ``Hello, World!'' didalamnya dan jalankan skrip.

Lakukan percobaan menulis program dengan bebearpa cara yang berbeda. Secara khusus, tulis setidaknya satu metode menggunakan modul Printf. Gunakan dokumentasi OCaml online untuk cukup memahami modul ini untuk melakukannya.
\end{ex}
